...
GenX can be precompiled to reduce the time it takes to run a case. To precompile GenX, you can follow these steps:
...
!!! note "Note"
    The environment variable `GENX_PRECOMPILE` must be set before loading GenX for the first time. To force the recompilation of GenX, you can delete the `~/.julia/compiled/vZ.Y/GenX/*.ji` binaries, set `GENX_PRECOMPILE` to `true`, and then reload the package (vZ.Y is the version of Julia you are using, e.g., v1.9). If GenX was imported via `Pkg.develop` or `] dev`, modifying the package files will also force recompilation.